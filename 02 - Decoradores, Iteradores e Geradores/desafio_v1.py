...
def log_transacao(func):
    def envelope(*args, **kwargs):
        ...
        nome_funcao = func.__name__.replace("_", " ").title()
        print(f"[{data_hora}] Função '{nome_funcao}' executada com sucesso!")
        return resultado
    return envelope
...
class ContaIterador:
    ...
    def __next__(self):
        if self._indice < len(self.contas):
            ...
            self._indice += 1
            return f"Agência: {conta.agencia} | Conta: {conta.numero} | Titular: {conta.cliente.nome}"
        raise StopIteration
...
class ContaCorrente(Conta):
    ...
    def sacar(self, valor):
        saques_dia = len([t for t in self.historico.transacoes if t["tipo"] == "Saque"])
        if valor > self._limite:
            print(f"\n@@@ Saque excede limite de R$ {self._limite:.2f} @@@")
            return False
        if saques_dia >= self._limite_saques:
...
def recuperar_conta_cliente(cliente):
    if not cliente.contas:
        ...
        return None
    return cliente.contas[0]  # Pode ser melhorado com escolha
...
def depositar(clientes):
    ...
    if not cliente: 
        ...
        return
    valor = float(input("Valor do depósito Rm: "))
...
def sacar(clientes):
    ...
    if not cliente: 
        print("Cliente não encontrado!")
        return
    valor = float(input("Valor do saque Rm: "))
    conta = recuperar_conta_cliente(cliente)
    if conta:
...
def exibir_relatorio(clientes):
    ...
    tipo = input("Filtrar por tipo? [S]aque / [D]epósito / [T]odos: ").strip().upper()
    if tipo == "S":
        filtro = "Saque"
    elif tipo == "D":
        filtro = "Deposito"
    ...
    print(f"\n=== RELATÓRIO DE TRANSAÇÕES ({filtro or 'Todas'}) ===")
    for t in conta.historico.gerar_relatorio(tipo_transacao=filtro):
        print(t)
